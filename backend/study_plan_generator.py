...
def _build_subjects_info(user_id: str) -> tuple[str, list[dict]]:
    ...
    for subj in subjects:
        ...
        days_left = 999  # default for no deadline
        if exam_date:
            try:
...
def _calculate_weeks_needed(enriched_subjects: list[dict]) -> int:
    ...
    # Cap at 2 weeks (14 days) to keep AI output within token limits and fast
...
def generate_and_save_plan(
    ...
    # Calibrate max_tokens to stay safely within Groq model token limits
    max_tokens = min(2000 + (total_weeks * 500), 3000)
    plan_data = ask_llm_json(prompt, temperature=0.5, max_tokens=max_tokens)

    # Handle cases where model wraps list in a dict
    if isinstance(plan_data, dict):
        for k in ["weeks", "plan", "study_plan", "data"]:
            if k in plan_data and isinstance(plan_data[k], list):
                plan_data = plan_data[k]
                break
